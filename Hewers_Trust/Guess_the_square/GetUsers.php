...
if($result->num_rows>0){while($row = $result->fetch_assoc()){
    echo "<script>UserSheet.pushUsers('".$row["firstname"]."','".$row['lastname']."',".$row["shares"].",".$row["fshares"].",".$row['Diamonds'].",'".$row["role"]."','".$row["phoneNumber"]."','".$row["email"]."', (CheckAge('".$row['birthday']."')) );</script>";}
}
